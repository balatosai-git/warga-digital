...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  if (!id) return NextResponse.json({ message: "ID peran tidak valid." }, { status: 400 });
  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ message: "Gagal mengubah peran." }, { status: 500 });
    }
    if (!data) {
...
export async function DELETE(_request: Request, context: RouteContext) {
  ...
  if (error) {
    ...
    return NextResponse.json({ message: "Gagal menghapus peran." }, { status: 500 });
  }
